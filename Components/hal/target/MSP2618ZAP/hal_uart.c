...
/* Set Source Clock */
#define HAL_UART_SET_SRC_CLK()         { UCA0CTL1 |= UCSSEL1; UCA0CTL1 &= ~UCSSEL0; } /* SMCLK */

/* Setup TXD and RXD Port */
#define HAL_UART_PORT_CONFIG()         { P3SEL |= BV(4) | BV(5); }      /* P3.4, P3.5 - UCA0TXD and RXD */

/* Setup format frame */
#define HAL_UART_FRAME_CONFIG()        { UCA0CTL0 &= ~UCPEN; UCA0CTL0 &= ~UCSPB; UCA0CTL0 &= ~UC7BIT; } /* 8N1 */

/* Enable/Disable TX */
#define HAL_UART_TX_ENABLE()           /* N/A */
#define HAL_UART_TX_DISABLE()          /* N/A */

/* Enable/Disable TX INT */
#define HAL_UART_TX_INT_ENABLE()       { IE2 |= UCA0TXIE; }
#define HAL_UART_TX_INT_DISABLE()      { IE2 &= ~UCA0TXIE; }
...
/* Enable/Disable TX INT */
#define HAL_UART_RX_INT_ENABLE()       { IE2 |= UCA0RXIE; }
#define HAL_UART_RX_INT_DISABLE()      { IE2 &= ~UCA0RXIE; }
...
#define HAL_UART_GET_RX_STATUS()      (IFG2 & UCA0RXIFG)
#define HAL_UART_GET_TX_STATUS()      (IFG2 & UCA0TXIFG)
#define HAL_UART_CLR_TX_STATUS()      (IFG2 &= ~UCA0TXIFG)

/* UART CTS and RTS */
#define HAL_UART_CTS_PORT              P2OUT
#define HAL_UART_CTS_BIT               BV(6)

#define HAL_UART_RTS_PORT              P2IN
#define HAL_UART_RTS_BIT               BV(7)

#define HAL_UART_FLOWCONTROL_INIT()    {P2DIR |= HAL_UART_CTS_BIT; P2DIR &= ~HAL_UART_RTS_BIT;}
...
void Hal_UART_FlowControlSet( uint8 port, uint8 status )
{
  if ( status == HAL_UART_FLOW_ON )
    HAL_UART_CTS_PORT &= ~HAL_UART_CTS_BIT;  /* Enable RX flow */
  else
    HAL_UART_CTS_PORT |= HAL_UART_CTS_BIT;   /* Disable RX flow */  
}
...
/**************************************************************************************************
 * @fn      INTERRUPT_UART0_TX_READY
 *
 * @brief   Called when xmtr shift register and buffer are both empty.
 *
 * @param   void
 *
 * @return  void
