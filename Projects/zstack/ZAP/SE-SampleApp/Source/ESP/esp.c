...
static uint8 esp_ProcessInReportCmd( zclIncomingMsg_t *pInMsg )
{
  ...
  for (j = 0; j < reportCmd->numAttr; j++)
  {
    // Device is notified of the latest values of the attribute of another device.
      reportRec = &(reportCmd->attrList[j]);
    
    if ( reportRec->attrID == ATTRID_SE_POWER_FACTOR ) 
   {
     meterData1 = reportRec->attrData;
     //  uint8 buf[25];

   // _itoa( meterData, &buf[0], 10 );
     HalLcdWriteString((char*)meterData1, HAL_LCD_LINE_4);
   }
   
       if ( reportRec->attrID == ATTRID_SE_CURRENT_SUMMATION_RECEIVED ) 
   {
     meterData2 = reportRec->attrData;
     //  uint8 buf[25];

   // _itoa( meterData, &buf[0], 10 );
     HalLcdWriteString((char*)meterData2, HAL_LCD_LINE_5);
   }
