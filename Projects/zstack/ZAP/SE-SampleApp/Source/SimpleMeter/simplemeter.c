...
static uint8 numAttr = 3;                                          // number of attributes in report
...
void simplemeter_Init( uint8 task_id )
{
  ...
  if ( pReportCmd != NULL )
  {
    ...
    pReportCmd->attrList[1].attrID = ATTRID_SE_POWER_FACTOR;
    pReportCmd->attrList[1].dataType = ZCL_DATATYPE_UINT48;
    
    
    pReportCmd->attrList[2].attrID = ATTRID_SE_CURRENT_SUMMATION_RECEIVED;
    pReportCmd->attrList[2].dataType = ZCL_DATATYPE_UINT48;
...
uint16 simplemeter_event_loop( uint8 task_id, uint16 events )
{
  ...
  if ( events & SIMPLEMETER_REPORT_ATTRIBUTE_EVT )
  {
    ...
    uint16 v,i;
     ...
     v = HalAdcRead (ADC12INCH_10,0);//
     i = HalAdcRead (ADC12INCH_11,0);
...
//    
    ...
    pReportCmd->attrList[1].attrData = buf_1;
    pReportCmd->attrList[2].attrData = buf_2; 
    
    HalLcdWriteString ( (char*)buf_1, 0x04);
    HalLcdWriteString ( (char*)buf_2, 0x05);
