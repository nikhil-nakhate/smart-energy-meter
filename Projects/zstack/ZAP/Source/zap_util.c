...
uint8 AddrMgrExtAddrLookup(uint16 nwkAddr, uint8* extAddr)
{
  ...
  if (SUCCESS == zapUtilReq(MT_UTIL_ADDRMGR_EXT_ADDR_LOOKUP, extAddr, args))
  {
 ...
 **************************************************************************************************
 */
associated_devices_t *AssocFindDevice(uint8 number)
{
  uint8 pBuf[sizeof(associated_devices_t)];

  if ((SUCCESS == zapUtilReq(MT_UTIL_ASSOC_FIND_DEVICE, pBuf, &number)) &&
...
uint8 zapUtilReq(uint8 cmd, uint8 *req, uint8 *args)
{
  ...
  case MT_UTIL_ADDRMGR_EXT_ADDR_LOOKUP:
    ...
    pBuf[Z_EXTADDR_LEN] = *args++;
    pBuf[Z_EXTADDR_LEN+1] = *args;
    break;
